...
export type SharePayload = {
  ...
  canvas?: SharedCanvas;
  /** Present when a resource page was shared. */
  resource?: string;
};
...
export const buildSharePayload = (
  ...
  if (isLocal(connection)) {
    return { v: 1, query, connection: { kind: "local" }, ...extras };
  }
  ...
  return { v: 1, query, connection: shared, ...extras };
};
...
export const parseShareFragment = (hash: string): SharePayload | undefined => {
  ...
  // Validated like any other IRI from an untrusted source.
  const resource =
    typeof payload.resource === "string" && isSafeIri(payload.resource)
      ? payload.resource
      : undefined;
  const extras = {
    ...(canvas ? { canvas } : {}),
...
export type AppliedShare = {
  ...
  canvas?: SharedCanvas;
  /** Handed to the resource view, which fetches it on arrival. */
  resource?: string;
  notice?: SharedNotice;
};
...
export type SharedNotice = {
  ...
  canvasName?: string;
  /** IRI the link asked to dereference, if any. */
  resource?: string;
};
...
export const applyShare = (
  ...
  if (payload.connection.kind === "local") {
    ...
    return {
      ...
      notice:
        payload.canvas || payload.resource
          ? {
